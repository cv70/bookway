...
面向大规模生产化的功能闭环、推荐数据演进与微服务拆分门槛见
[`docs/production-closed-loop.md`](docs/production-closed-loop.md)。该蓝图以现有
服务事实边界为准，避免因功能扩展重复创建数据所有权。
...
底部使用 5 个固定入口，兼顾个人行动和公共发现，中央为创作按钮。今日负责当天执行，路线负责长期规划与管理，两者保持独立；路线不归入“我的”。

| 导航 | 图标建议 | 作用 |
| --- | --- | --- |
| 今日 | `House` | 今日行动、精力状态、最近进展与小结 |
| 发现 | `Compass` | 推荐、关注、搜索、主题与附近路线 |
| 创作 | `Plus` | 快速记录，或将记录发布为行记 |
| 路线 | `Route` | 创建和管理草稿、进行中、暂停、已完成路线 |
...
- 无路线的新用户默认进入“发现”的精选内容流，并持续展示“开始第一条路线”的明确入口；兴趣信号可在浏览、搜索和加入路线时渐进收集。
- 已有个人路线的用户默认进入“今日”；有行动时优先执行，无行动时引导从既有路线安排下一步，避免内容消费挤占实际行动。
...
首次使用不设置独立的新手流程。用户可直接浏览发现页、搜索内容、创建空白路线或加入公共路线；在创建路线时才渐进配置必要信息。

1. 无路线用户进入“发现”，可浏览精选内容、搜索目标，或直接创建空白路线。
2. 用户可从公共路线生成私人副本，也可使用低门槛模板或从空白路线开始。
3. 创建路线时设置名称、周期、首个阶段和下一步行动；领域、当前状态与每周节奏均可跳过并在后续修改。
4. 创建或加入后进入路线详情，并在“今日”获得第一个可执行行动。
5. 先进入应用再引导注册；需要云同步时再绑定手机号、邮箱或系统账号。
...
冷启动阶段用编辑精选、浏览与搜索意图、加入路线等渐进信号，不需要机器学习；拥有足够高质量反馈后再引入特征平台和模型。健康、未成年人和敏感个人信息不得用于广告或不必要的推荐画像。
