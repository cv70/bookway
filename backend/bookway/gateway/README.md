...
创建 `content_type=milestone` 的打卡复盘时，Gateway 会先向 BBS 查询当前用户对目标公开路线的活动参与状态；未加入、已退出或参与事实不可用都会 fail-closed，拒绝写入。BBS Link 继续负责公开路线/阶段快照和审核状态，客户端不能通过伪造路线标题、阶段标题或私人行动 ID 制造完成证据。
...
`GATEWAY_ADDR`、`STORAGE_MODE`、`DATABASE_URL`、`ACCOUNT_GRPC_URL`、`GROWTH_GRPC_URL`、`BBS_FEED_GRPC_URL`、`BBS_LINK_GRPC_URL`、`SEARCH_MAIN_GRPC_URL`、`USER_EVENT_GRPC_URL`、`BBS_GRPC_URL`、`BBS_CREATOR_GRPC_URL`、`BBS_MESSAGE_GRPC_URL`、`COMMENT_GRPC_URL`、`INTERACTION_STATUS_GRPC_URL`、`MEDIA_GRPC_URL`、`CONTENT_AUDIT_GRPC_URL`、`FEEDBACK_GRPC_URL`、`AD_MAIN_GRPC_URL`、`MALL_GRPC_URL`、`MALL_ORDER_GRPC_URL`、`AUTH_REQUIRED`、`AUTH_JWT_SECRET`、`AUTH_JWKS_URL`、`AUTH_ISSUER`、`AUTH_AUDIENCE`、`AUTH_JWKS_CACHE_SECONDS`、`SERVICE_AUTH_TOKEN`、`HTTP_CONNECT_TIMEOUT_MS`、`HTTP_REQUEST_TIMEOUT_MS`、`REDIS_URL`、`REDIS_CONNECT_TIMEOUT_MS`、`REDIS_COMMAND_TIMEOUT_MS`、`RATE_LIMIT_PER_MINUTE`。`STORAGE_MODE=postgres` 时 Gateway 需要 `DATABASE_URL` 以提交社区通知任务；生产鉴权建议使用 OIDC：配置 `AUTH_JWKS_URL`，并可选地约束 `AUTH_ISSUER` 与 `AUTH_AUDIENCE`，JWKS 会按 TTL 缓存并在未知 `kid` 时立即刷新。未配置 JWKS 时才使用 HS256 `AUTH_JWT_SECRET`（本地开发/联调）；审核 JWT 的 `roles` 声明是字符串数组；角色由身份系统签发，不能由客户端请求头提供。

## 生产化待办

当前已接入 OIDC/JWKS（含 TTL 缓存和未知 `kid` 轮换刷新）、HS256 本地回退、服务令牌、请求 ID、Redis 限流和统一调用超时。下一阶段补齐接口级限流策略、OpenTelemetry 上下文传播、OpenAPI 契约和分接口容量压测。
