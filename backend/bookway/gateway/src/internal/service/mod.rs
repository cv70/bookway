...
use super::{
    ...
    datasource::UpstreamError,
    domain::GatewayService,
};

#[derive(Clone)]
pub(crate) struct AppState {
    pub(crate) gateway: GatewayService,
}
...
async fn list_journeys(
    ...
    Ok(Json(ApiResponse::new(
        state.gateway.list_journeys(&user_id(&headers)).await?,
    )))
}
...
async fn today(
    ...
    Ok(Json(ApiResponse::new(
        state.gateway.today(&user_id(&headers)).await?,
    )))
}
...
async fn feed(
    ...
    Ok(Json(ApiResponse::new(state.gateway.feed(request).await?)))
}
...
async fn search(
    ...
    request.user_id = Some(user_id(&headers));
    Ok(Json(ApiResponse::new(state.gateway.search(request).await?)))
}
...
async fn suggestions(
    ...
    Ok(Json(ApiResponse::new(
        state.gateway.suggestions(&request.q).await?,
    )))
}
...
async fn get_media(
    ...
    Ok(Json(ApiResponse::new(
        state.gateway.get_media(&user_id(&headers), &id).await?,
    )))
}
...
async fn get_content(
    ...
    Ok(Json(ApiResponse::new(
        state.gateway.get_content(&id).await?,
    )))
}
...
async fn list_comments(
    ...
    Ok(Json(ApiResponse::new(
        state.gateway.comments(&post_id).await?,
    )))
}
