...
impl IntoResponse for HttpError {
    fn into_response(self) -> Response {
        let (status, code) = match &self.0 {
            ...
            MessageError::Repository(crate::datasource::DaoError::NotFound(_)) => {
                (StatusCode::NOT_FOUND, "conversation_not_found")
            }
            MessageError::Repository(crate::datasource::DaoError::NotParticipant) => {
                (StatusCode::FORBIDDEN, "conversation_access_denied")
            }
            MessageError::Repository(crate::datasource::DaoError::IdempotencyConflict) => {
