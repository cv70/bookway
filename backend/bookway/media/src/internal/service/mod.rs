...
use super::{
    ...
    domain::{MediaError, MediaService},
};
...
pub(crate) fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route("/internal/v1/media/upload-url", post(create_upload))
        .route("/internal/v1/media/{id}", get(get_media))
        .route("/internal/v1/media/{id}/complete", post(complete_upload))
        .with_state(state)
...
async fn complete_upload(
    ...
    Ok(Json(ApiResponse::new(
        state.media.complete_upload(&user_id(&headers), &id).await?,
    )))
}
async fn get_media(
    State(state): State<AppState>,
    ...
    Ok(Json(ApiResponse::new(
        state.media.get(&user_id(&headers), &id).await?,
    )))
}
