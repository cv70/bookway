...
候选的 pWEGU、pCTR、pCVR 使用近 90 天总体行为作为平滑先验，再按当前用户的有效曝光和加入样本量逐步切换到个性化比率。路线使用 complete / join_route，知识内容使用 complete / save_knowledge；冷启动路线因此不会被当成零转化，非路线内容不会获得路线完成或购买信号。
