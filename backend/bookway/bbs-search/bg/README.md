...
写索引必须由 `OPENSEARCH_WRITE_INDEX` 显式指定为物理索引，不能配置为读别名或数据流；Worker 会在任何文档写入前拒绝这两种配置。主、影子索引任一写入失败都会保留 Outbox 任务重试；同一内容版本以 OpenSearch `external_gte` 写入，因此重试、重建和双写不会让旧投影覆盖新投影。发布新索引时先配置影子写入并完整重建目标，再由 [`../job/README.md`](../job/README.md) 的别名切换任务发布。
