...
use crate::datasource::semantic::SemanticRecallDataSource;

#[derive(Clone)]
pub struct Domain {
    pub(crate) config: Config,
    pub(crate) content_client: BbsLinkClient<tonic::transport::Channel>,
    pub(crate) semantic: Option<SemanticRecallDataSource>,
...
impl Domain {
    pub async fn new(config: Config) -> Result<Self, bookway_runtime::ConnectFailure> {
        ...
        let semantic = match config.semantic.as_ref() {
            Some(semantic_config) => Some(SemanticRecallDataSource::connect(semantic_config).await?),
            None => None,
        };
        Ok(Self {
            content_client,
