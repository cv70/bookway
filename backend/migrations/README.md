...
| `0067_route_node_resource_attachments.sql` | knowledge-catalog | 路线行动节点资源挂载、RAG 检索集合预留、幂等创建和软归档 |
...
`0067` 把路线行动节点与公共资源的挂载关系收敛在 Knowledge Catalog 限界上下文中。挂载记录只引用公开资源和公开路线节点标识，不复制路线树或资源正文；`embedding_collection` 为后续 RAG/AI 行动指南提供稳定检索集合名。
