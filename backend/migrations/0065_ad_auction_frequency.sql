...
ALTER TABLE ad_campaigns
    ADD COLUMN IF NOT EXISTS predicted_ctr DOUBLE PRECISION NOT NULL DEFAULT 0.01
        CHECK (predicted_ctr >= 0 AND predicted_ctr <= 1),
    ADD COLUMN IF NOT EXISTS predicted_cvr DOUBLE PRECISION NOT NULL DEFAULT 0.01
        CHECK (predicted_cvr >= 0 AND predicted_cvr <= 1),
    ADD COLUMN IF NOT EXISTS global_frequency_cap INTEGER NOT NULL DEFAULT 0
