...
CREATE TABLE IF NOT EXISTS mall_node_offers (
    ...
    CHECK (length(route_id) > 0 AND length(action_node_id) > 0 AND length(creator_id) > 0)
);
...
ALTER TABLE mall_orders ADD COLUMN IF NOT EXISTS node_offer_id TEXT REFERENCES mall_node_offers(id);
ALTER TABLE mall_orders ADD COLUMN IF NOT EXISTS affiliate_creator_id TEXT;
