...
| 公开分享与社区 | `bbs-link` 内容状态机、`media` 资产门禁、`comment`、`interaction-status`、`bbs`、`bbs-message` | 内容/评论/私信均有举报、审核或申诉边界；拉黑和静音在读路径 fail-closed；移动端创作者主页和个人页均可进入私信会话 |
...
的私有知识库。资源版权、引用与第三方来源信息由 `knowledge-catalog` 公共目录拥有，目录只返回已发布资源，并要求 provider、canonical URL、license、version 和 citation；私有 Growth 资源不会自动进入公共目录。
...
派生的事件计算，缺少分母时安全回退为零，不用客户端自报的“完成”或“购买”充数。

### 场景化商城

`mall` 拥有商品、SKU 与“公共路线 + Action Node -> SKU”的 `NodeOffer` 关联；它不
...
稳定键写入归因到公共路线的 `purchase` 事件；这个写入降级不会回滚已完成支付。
...
首次确认时的指标快照；再次编辑只修改用户文字，不会篡改历史回望。确认建议时，
`POST /v1/reviews/{review_id}/adjustments/{suggestion_index}/apply` 由 `growth` 在同一
事务中锁定复盘与目标、校验首次快照中的预期状态、更新 Journey/Action 并保存决定。
调用方不能提交任意 patch；重复请求返回原决定，目标改变后安全拒绝陈旧建议。
...
内容服务现有的笔记、文章、视频、路线和阶段成果足以承接首发 UGC。阶段成果已
落在 `bbs-link`：它只关联当前公开路线的公开阶段快照，记录投入、结果、调整和
证据范围，不读取私人 Journey/Action/Entry；它沿用内容审核、可见性、搜索 Outbox
和 Feed 召回。问题与答案已按同一内容事实边界落地，而非泛化更多流量型模板：

| 内容形态 | 要解决的问题 | 最小结构 | 主要动作 |
| --- | --- | --- | --- |
| 阶段成果（已实现） | 让别人判断路线是否真的适合自己 | 路线、阶段、投入、结果、调整、证据范围 | 查看路线、比较版本、采用轻量版 |
| 问题与答案（已实现） | 在执行受阻时获得可复用的帮助 | 领域、可选公开路线/阶段快照、问题、答案状态、可选采纳结果 | 关联路线、追问、标记已解决 |

它们应扩展 `bbs-link` 的内容契约，评论继续只拥有回答正文、层级、审核和举报。
“采纳答案”“问题已解决”等与内容状态有关的事实由 BBS Link 持有，不能由评论
服务反向拥有帖子状态。问题关联路线时，BBS Link 只读取当前公开且同领域的路线，
并固定路线与可选阶段标题快照，绝不读取私人 Journey、Action、Entry、日程或进度。
所有公开互动延续拉黑/静音、审核、举报、申诉和通知 Outbox 的既有安全链路。
...
bbs-feed (产品 surface)
  -> recommend-main (编排、可见性、补全、多样性、曝光)
      ...
      -> bbs-link / bbs / interaction-status (权威事实批量补全)
```
...
2. `feature-main/job/feature-snapshot` 已先落地用户行为快照：从已验证事件和公开
   内容领域聚合窗口，写入版本、特征时间、血缘和 TTL；后续继续扩展内容、路线和
   作者特征，并将快照消费接入训练宽表。
