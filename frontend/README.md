...
生产或鉴权联调时通过 `EXPO_PUBLIC_AUTH_TOKEN` 提供短期 Bearer JWT；未设置时客户端保持匿名，不发送合成用户身份，只能使用公开读取能力。

原生推送注册还需要将 `EXPO_PUBLIC_EAS_PROJECT_ID` 配置为 EAS project ID；未配置时仍可使用应用内通知收件箱，但客户端不会请求或提交 Expo token。
...
- 本地开发默认保持匿名；Feed/Search 不接受客户端 `user_id` 查询参数，登录后才由 Bearer JWT 经 Gateway 注入可信身份。
- 登录后的曝光、点赞、搜索提交、路线加入、负反馈、举报和行动完成会批量上报 `/v1/events`，支持 100 条 flush、定时 flush、后台 flush、重试和曝光去重；匿名浏览不会伪造用户事件。
...
- 搜索已能展示内容、用户、主题、公开资源、路线行动节点和场景装备结果，并保留会话内历史；支持动态联想、查询绑定游标、加载更多与近底部续页。节点/装备结果保留其所属路线卡，搜索广告使用同一场景广告回执与路线节点约束。输入和联想请求会丢弃过期响应。后端 PIT 搜索会话过期时，客户端会从第一页刷新查询而不是反复请求失效游标；搜索服务失败时只在当前已加载内容中降级查找，并明确标记降级结果。
- Feed 服务失败会保留空结果并明确显示降级状态，不再回退到共享演示用户或演示数据；生产构建仍需要把 Feed 和搜索的降级状态接入可观测性，避免掩盖长时间后端故障。
- 资源知识库与阅读状态已接入 Gateway；EPUB/PDF/MOBI 文件导入、Foliate/WebView 渲染、离线冲突合并和增量同步仍需要独立阅读引擎与文件服务。
- 行动留痕、周回望和陪伴简报已从客户端临时状态迁移到 Gateway；离线写入队列、冲突合并和跨设备增量同步仍需实现。创建和新增行动可选择今天/明天及具体时间，客户端会提交带 UTC 偏移量的时间戳和 IANA 时区；今日和陪伴请求也会携带本地日期上下文。通知中心已读取并确认服务端收件箱，未读数会显示在今日页铃铛上，支持游标续页、近底部加载和失败重试；含 `post_id` 的社区通知会优先打开已缓存帖子，未命中时通过公开内容接口加载，加载失败会明确显示并可重试。含 `appeal_id` 的申诉结果会改为打开创作中心，避免尝试读取受限内容的公开详情。设置页的行动提醒会持久化启用状态和设备本地时区；登录后的原生 iOS/Android 客户端会按 Expo SDK 57 权限与 EAS project ID 要求注册 Expo push token，并在权限、project ID 或设备能力不可用时安全跳过。自动重排仍需实现。
