...
type Props = {
  ...
  onOpenRouteDraft: (content: OwnedContent) => void;
...
export function ProfileSectionModal({ section, visible, journeys, entries, savedPosts, review, onApplyReviewSuggestion, onRetryEntryPublication, onOpenRouteDraft, retryingEntryIds, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    void getDirectMessagePreferences()
      .then((preferences) => { if (active) setAllowDirectMessages(preferences.allow_direct_messages); })
      .catch(() => undefined);
    return () => { active = false; };
  }, [section, visible]);
  const setDirectMessagePreference = (enabled: boolean) => {
    setAllowDirectMessages(enabled);
    setDirectMessagesSaving(true);
    void updateDirectMessagePreferences(enabled)
      .catch(() => setAllowDirectMessages(!enabled))
      .finally(() => setDirectMessagesSaving(false));
  };
  useEffect(() => {
    ...
    <Modal animationType="slide" onRequestClose={onClose} visible={visible}>
      <View style={styles.screen}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {section === 'settings' ? <SettingsPanel allowDirectMessages={allowDirectMessages} analytics={analytics} directMessagesSaving={directMessagesSaving} exportPreview={exportPreview} notificationStatus={reminderError ?? (remindersLoading ? '正在读取提醒设置…' : remindersSaving ? '正在保存提醒设置…' : undefined)} notifications={notifications} remindersBusy={remindersLoading || remindersSaving} setAllowDirectMessages={setDirectMessagePreference} setAnalytics={setAnalytics} setNotifications={setNotificationsPreference} /> : null}
        </ScrollView>
      </View>
    </Modal>
  );
...
function Creation({ appeals, appealsLoadMoreError, appealsLoadingMore, appealsNextCursor, contentManagementError, contentManagementLoading, contents, contentsLoadMoreError, contentsLoadingMore, contentsNextCursor, entries, onLoadMoreAppeals, onLoadMoreContents, onOpenRouteDraft, onRefreshContentManagement, onRetryEntryPublication, onSubmitAppeal, retryingEntryIds, submittingContentId }: { appeals: ContentAppeal[]; appealsLoadMoreError?: string; appealsLoadingMore: boolean; appealsNextCursor?: string; contentManagementError?: string; contentManagementLoading: boolean; contents: OwnedContent[]; contentsLoadMoreError?: string; contentsLoadingMore: boolean; contentsNextCursor?: string; entries: GrowthEntry[]; onLoadMoreAppeals: () => void; onLoadMoreContents: () => void; onOpenRouteDraft: (content: OwnedContent) => void; onRefreshContentManagement: () => void; onRetryEntryPublication: (entryId: string) => Promise<void>; onSubmitAppeal: (contentId: string, details: string) => Promise<void>; retryingEntryIds: Set<string>; submittingContentId?: string }) {
...
function SettingsPanel({ notifications, setNotifications, analytics, setAnalytics, allowDirectMessages, setAllowDirectMessages, directMessagesSaving, exportPreview, remindersBusy, notificationStatus }: { notifications: boolean; setNotifications: (value: boolean) => void; analytics: boolean; setAnalytics: (value: boolean) => void; allowDirectMessages: boolean; setAllowDirectMessages: (value: boolean) => void; directMessagesSaving: boolean; exportPreview: string; remindersBusy: boolean; notificationStatus?: string }) {
  const [showExport, setShowExport] = useState(false);
  return <><SettingRow disabled={remindersBusy} label="行动提醒" description="保存提醒偏好；启用系统通知后可按安排提醒" value={notifications} onChange={setNotifications} />{notificationStatus ? <Text style={styles.settingStatus}>{notificationStatus}</Text> : null}<SettingRow disabled={directMessagesSaving} label="接收私信" description="允许其他用户从创作者主页联系你" value={allowDirectMessages} onChange={setAllowDirectMessages} /><SettingRow label="匿名使用数据" description="用于改善推荐和稳定性" value={analytics} onChange={setAnalytics} /><Pressable onPress={() => setShowExport((value) => !value)} style={({ pressed }) => [styles.export, pressed && styles.pressed]}><Download color={colors.evergreen} size={19} /><Text style={styles.exportText}>查看数据导出预览</Text></Pressable>{showExport ? <Text selectable style={styles.exportPreview}>{exportPreview}</Text> : null}</>;
}
...
const styles = StyleSheet.create({
  ...
  routeDraft: { minHeight: 78, padding: 13, flexDirection: 'row', alignItems: 'center', gap: 12, borderWidth: 1, borderColor: colors.evergreenSoft, borderRadius: 7, backgroundColor: colors.surface },
  routeDraftCopy: { flex: 1, minWidth: 0, gap: 5 },
  routeDraftTitle: { color: colors.ink, fontSize: 13, fontWeight: '700', lineHeight: 19, letterSpacing: 0 },
  routeDraftMeta: { color: colors.muted, fontSize: 11, lineHeight: 17, letterSpacing: 0 },
  routeDraftButton: { minHeight: 34, paddingHorizontal: 10, alignItems: 'center', justifyContent: 'center', borderRadius: 5, backgroundColor: colors.evergreen },
  routeDraftButtonText: { color: colors.surface, fontSize: 11, fontWeight: '700', letterSpacing: 0 },
