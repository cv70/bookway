...
export function PostDetailModal({
  ...
  const [acceptedAnswerId, setAcceptedAnswerId] = useState<string>();
  const [acceptingAnswerId, setAcceptingAnswerId] = useState<string>();
  const [acceptAnswerError, setAcceptAnswerError] = useState(false);
  ...
  useEffect(() => {
    ...
    setAcceptedAnswerId(content?.accepted_answer_id ?? undefined);
    setAcceptingAnswerId(undefined);
    setAcceptAnswerError(false);
  }, [post?.id, visible, content?.accepted_answer_id]);
  if (!post) return null;
  ...
  const questionContext = content?.question_context;
  ...
  const acceptAnswer = async (item: Comment) => {
    setAcceptAnswerError(false);
    setAcceptingAnswerId(item.id);
    try {
      const updated = await onAcceptAnswer(post.id, item.id);
      setAcceptedAnswerId(updated.accepted_answer_id ?? item.id);
    } catch {
      setAcceptAnswerError(true);
    } finally {
      setAcceptingAnswerId(undefined);
    }
  };
    ...
    <Modal animationType="slide" onRequestClose={onClose} visible={visible}>
      <View style={styles.screen}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.commentHeader}><Text style={styles.commentTitle}>{isQuestion ? '回答' : '评论'}</Text><Text style={styles.commentMeta}>{hasMoreComments ? `已加载 ${comments.length} 条` : `${comments.length} ${isQuestion ? '个回答' : '条'}`}</Text></View>
          ...
          <View style={styles.commentComposer}>
            <TextInput maxLength={1000} multiline onChangeText={(value) => { setComment(value); setCommentError(false); setCommentNotice(''); }} placeholder={replyTo ? `回复 ${replyTo.author_name}` : isQuestion ? '写下你的回答' : '写下你的回应'} placeholderTextColor={colors.faint} style={styles.commentInput} value={comment} />
            <Pressable accessibilityLabel={isQuestion ? "发布回答" : "发布评论"} disabled={!comment.trim() || commentSubmitting} onPress={() => void submitComment()} style={[styles.send, (!comment.trim() || commentSubmitting) && styles.sendDisabled]}><Send color={colors.surface} size={16} /></Pressable>
          </View>
          ...
          <View style={styles.commentList}>
            {comments.length === 0 ? <Text style={styles.empty}>{isQuestion ? '成为第一个写下回答的人' : '成为第一个留下回应的人'}</Text> : comments.map((item) => {
              ...
              const isAccepted = acceptedAnswerId === item.id;
              const canAccept = isQuestion && currentUserId === content?.author_id && !item.parent_id && isPublic && !isDeleted && !item.id.startsWith('local-comment-');
              return <View key={item.id} style={[styles.comment, item.parent_id && styles.commentReply, isAccepted && styles.acceptedAnswer]}><View style={styles.commentAvatar}><Text style={styles.commentInitial}>{item.author_name.slice(0, 1)}</Text></View><View style={styles.commentBody}>{isAccepted ? <View style={styles.acceptedBadge}><CheckCircle2 color={colors.evergreen} size={14} /><Text style={styles.acceptedBadgeText}>已采纳答案</Text></View> : null}<Text style={styles.commentAuthor}>{item.author_name}{parent ? <Text style={styles.replyAuthor}> 回复 {parent.author_name}</Text> : null}</Text><Text style={styles.commentText}>{item.body}</Text><View style={styles.commentFooter}><Text style={styles.commentDate}>{isDeleted ? '已删除' : item.status === 'reviewing' ? '审核中' : item.status === 'restricted' ? '未公开' : formatDate(item.created_at)}</Text><Pressable accessibilityLabel={`回复 ${item.author_name}`} disabled={item.id.startsWith('local-comment-') || !isPublic || isDeleted} hitSlop={6} onPress={() => { setReplyTo(item); setCommentError(false); setCommentNotice(''); }}><Text style={[styles.replyButton, (!isPublic || isDeleted) && styles.replyButtonDisabled]}>回复</Text></Pressable>{canAccept ? <Pressable accessibilityLabel="采纳为答案" disabled={acceptingAnswerId === item.id} hitSlop={6} onPress={() => void acceptAnswer(item)}><Text style={[styles.acceptAnswerButton, acceptingAnswerId === item.id && styles.replyButtonDisabled]}>{acceptingAnswerId === item.id ? '采纳中' : isAccepted ? '已采纳' : '采纳'}</Text></Pressable> : null}{canDelete ? <Pressable accessibilityLabel="删除评论" disabled={deletingCommentId === item.id} hitSlop={6} onPress={() => confirmDeleteComment(item)}><Text style={[styles.deleteCommentButton, deletingCommentId === item.id && styles.replyButtonDisabled]}>{deletingCommentId === item.id ? '删除中' : '删除'}</Text></Pressable> : null}</View></View></View>;
            })}
            {acceptAnswerError ? <Text accessibilityLiveRegion="polite" style={styles.commentError}>采纳失败，请稍后重试</Text> : null}
...
function MilestoneField({ label, value }: { label: string; value: string }) {
  return <View style={styles.milestoneField}><Text style={styles.milestoneLabel}>{label}</Text><Text style={styles.milestoneValue}>{value}</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  milestone: { marginHorizontal: 20, marginTop: 17, padding: 15, gap: 10, borderRadius: 7, backgroundColor: colors.goldSoft, borderLeftWidth: 3, borderLeftColor: colors.gold },
  milestoneTitle: { color: colors.ink, fontSize: 15, fontWeight: '700', lineHeight: 21, letterSpacing: 0 },
  milestoneStage: { color: colors.gold, fontSize: 12, fontWeight: '700', letterSpacing: 0 },
  milestoneField: { gap: 3 },
  milestoneLabel: { color: colors.muted, fontSize: 11, fontWeight: '700', letterSpacing: 0 },
  milestoneValue: { color: colors.ink, fontSize: 13, lineHeight: 20, letterSpacing: 0 },
  questionContext: { marginHorizontal: 20, marginTop: 17, padding: 14, gap: 4, borderLeftWidth: 3, borderLeftColor: colors.blue, borderRadius: 7, backgroundColor: colors.blueSoft },
  questionContextLabel: { color: colors.blue, fontSize: 11, fontWeight: '700', letterSpacing: 0 },
  questionContextTitle: { color: colors.ink, fontSize: 14, fontWeight: '700', lineHeight: 20, letterSpacing: 0 },
  questionContextStage: { color: colors.muted, fontSize: 12, lineHeight: 18, letterSpacing: 0 },
  ...
  acceptedAnswer: { paddingLeft: 8, borderLeftWidth: 3, borderLeftColor: colors.evergreen, backgroundColor: colors.evergreenSoft },
  acceptedBadge: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 5 },
  acceptedBadgeText: { color: colors.evergreen, fontSize: 11, fontWeight: '700', letterSpacing: 0 },
  acceptAnswerButton: { color: colors.evergreen, fontSize: 10, fontWeight: '700', letterSpacing: 0 },
